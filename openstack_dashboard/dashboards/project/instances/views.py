    ...
    def get_data(self):
        ...
        try:
            if sort_dirs is not None:
                instances,self._more,self._prev = api.nova.server_list(
                    self.request,
                    search_opts=search_opts,sort_dirs=sort_dirs)
            else:
...
class DetailView(tabs.TabbedTableView):
    ...
    # patch by longxing, inherited from TabView to TabbedTableView to support
    # post requirements
...
class CreateSnapshotAdvancedView(forms.ModalFormView):
    # patch by longxing: view for create snapshots of instance attached disk
    form_class = project_forms.CreateSnapshotAdvanced
    modal_header = _("Create Snapshot")
    template_name = 'project/instances/x_create_snapshot_advanced.html'
    success_url = reverse_lazy("horizon:project:instances:index")
    modal_id = "create_snapshot_advanced_modal"
    submit_label = _("Create Snapshot")
    submit_url = "horizon:project:instances:create_snapshot_advanced"
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            exceptions.handle(self.request,
                              _('Unable to retrieve tenant quota infomation.'))
            context['usages'] = {}

        return context


class RollbackSnapshotAdvancedActView(forms.ModalFormView):
    # Patch by longxing:rollback a snapshot of an instance's disk
    form_class = project_forms.RollbackSnapshotAdvancedAct
    template_name = 'project/instances/rollback_snapshot_advanced.html'
    success_url = reverse_lazy("horizon:project:instances:index")
    modal_id = "rollback_snapshot_advanced_modal"
    modal_header = _("Rollback Snapshot")
    submit_label = _("Rollback Snapshot")
    submit_url = "horizon:project:instances:rollback_snapshot_advanced_act"
    ...
    def get_context_data(self, **kwargs):
        context = super(
            RollbackSnapshotAdvancedActView,
            self).get_context_data(
            **kwargs)
        context['instance_id'] = self.kwargs['instance_id']
