...
class CreateSnapshotAdvancedAction(tables.LinkAction):
    # Patch by longxing
    name = "createsnapshotadvanced"
    verbose_name = _("Create Snapshot Advanced")
    url = "horizon:project:instances:create_snapshot_advanced"
    classes = ("ajax-modal",)
    icon = "camera"
...
class ShowSnapshotAdvancedAction(policy.PolicyTargetMixin, tables.LinkAction):
    # Patch by longxing
    # redirect to instance snapshots details page
    name = "showsnapshotadvanced"
    verbose_name = _("Show Snapshot Advanced")
    url = "horizon:project:instances:detail"
    classes = ("btn-log",)
    icon = "camera"
    policy_rules = (("compute_extension:snapshots"),)

    def get_link_url(self, datum):
        base_url = super(ShowSnapshotAdvancedAction, self).get_link_url(datum)
        tab_query_string = "=".join(("tab", "instance_details__snapshots_tab"))
        return "?".join([base_url, tab_query_string])


class RollbackSnapshotAdvancedAction(
        policy.PolicyTargetMixin,
        tables.LinkAction):
    # Patch by longxing
    # redirect to instance snapshots details page
    name = "rollbacksnapshotadvanced"
    ...
    def get_link_url(self, datum):
        base_url = super(
            RollbackSnapshotAdvancedAction,
            self).get_link_url(datum)
        '''print datum.__dict__
        print dir(datum)
        print datum._attrs
        print tabs.LogTab(
            tabs.InstanceDetailTabs)
        print tabs.SnapshotTab(tabs.InstanceDetailTabs,self.request)'''
        tab_query_string = "=".join(("tab", "instance_details__snapshots_tab"))
        # print "?".join([base_url, tab_query_string])
        return "?".join([base_url, tab_query_string])
...
class InstancesTable(tables.DataTable):
    ...
    class Meta(object):
        ...
        row_actions = (
            CreateSnapshotAdvancedAction,
            ShowSnapshotAdvancedAction,
            RollbackSnapshotAdvancedAction,
            StartInstance,
            ConfirmResize,
            RevertResize,
