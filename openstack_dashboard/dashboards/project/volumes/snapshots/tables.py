...
class EditVolumeSnapshot(policy.PolicyTargetMixin, tables.LinkAction):
    ...
    def get_link_url(self, datum):
        ...
        params = urlencode({"instance_id":0,"type":"volumes"})
        return "?".join([base_url,params])
...
class RollBackSnapshot(tables.LinkAction):
    #patch by longxing, rollback a snapshot based on a volume
    name = "rollback_snapshot"
    verbose_name = _("Rollback Volume")
    url = "horizon:project:volumes:snapshots:rollback"
    classes = ("ajax-modal",)
    icon = "camera"
    policy_rules = (("volume", "volume:create"),)

    def get_link_url(self, datum):
        base_url = reverse(self.url,args=(datum.volume_id,datum.id,))
        return base_url
...
class VolumeSnapshotsTable(volume_tables.VolumesTableBase):
    ...
    class Meta(object):
        ...
        row_actions = ((CreateVolumeFromSnapshot,) + launch_actions +
                       (EditVolumeSnapshot, DeleteVolumeSnapshot,RollBackSnapshot))
        row_class = UpdateRow
