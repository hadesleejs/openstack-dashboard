...
(function () {
  ...
  function lbaasv2API(apiService, toastService) {
    ...
    function createLoadBalancer(spec) {
      horizon.modals.status=1;
      var res=apiService.post('/api/lbaas/loadbalancers/', spec);
       res.success(function(response){
        if(response){
          horizon.modals.spinner.modal('hide');
          horizon.modals.status=0;
        }
      })
       return res.error(function () {
    ...
    function editLoadBalancer(id, spec) {
      horizon.modals.status=1;
       var res=apiService.put('/api/lbaas/loadbalancers/' + id + '/', spec);
            res.success(function(response){
                  if(response){
                    horizon.modals.spinner.modal('hide');
                    horizon.modals.status=0;
    ...
    function createListener(spec) {
      horizon.modals.status=1;
      var res=apiService.post('/api/lbaas/listeners/', spec)
          res.success(function(response){
            if(response){
              horizon.modals.spinner.modal('hide');
              horizon.modals.status=0;
    ...
    function editListener(id, spec) {
        horizon.modals.status=1;
       var res=apiService.put('/api/lbaas/listeners/' + id + '/', spec);
           res.success(function(response){
              if(response){
                 horizon.modals.spinner.modal('hide');
                 horizon.modals.status=0;
    ...
    function getPools(id) {
      var params = id ? {'params': {'listenerId': id}} : {};
    ...
    function createPool(spec) {
      horizon.modals.status=1;
      var res=apiService.post('/api/lbaas/pools/', spec);
          res.success(function(response){
            if(response){
              horizon.modals.spinner.modal('hide');
              horizon.modals.status=0;
    ...
    function editPool(id, spec) {
      horizon.modals.status=1;
      var res=apiService.put('/api/lbaas/pools/' + id + '/', spec);
          res.success(function(response){
                if(response){
                  horizon.modals.spinner.modal('hide');
                  horizon.modals.status=0;
    ...
    function editMember(poolId, memberId, spec) {
       var res=apiService.put('/api/lbaas/pools/' + poolId + '/members/' + memberId + '/', spec)
            res.success(function(response){
                  if(response){
                    horizon.modals.spinner.modal('hide');
                  }
    ...
    function updateMemberList(poolId, spec) {
      var id=spec.loadbalancer_id;
      // console.log("++++++++");
      // var response=apiService.get('/api/lbaas/loadbalancers/' + id + '/');
      // console.log("============================");
      // console.log(response.data.provisioning_status);
      // console.log("============================");
      // getLoadBalancer(id).then(function onLoad(response) {
      //   console.log("============================");
      ...
      //   }
      // });
      // getPool(poolId).then(function onLoad(response) {
      //   console.log("*****************************");
      //   console.log(response.data);
      //   console.log("*****************************");
      //   if (['ACTIVE', 'ERROR'].indexOf(response.data.provisioning_status) > 0) {
      ...
      var res=apiService.put('/api/lbaas/pools/' + poolId + '/members/', spec);
      res.success(function(response){
        if(response=='ok'){
          horizon.modals.spinner.modal('hide');
        }
     ...
     * @description
     * Get  health monitors which is equal to poolId .
     * @param {string} poolId
     * Specifies the id of the pool.
     * add by yangsong.
     */

    function getHealthMonitors(poolId) {
      var params = poolId ? {'params': {'poolId': poolId}} : {};
    ...
    function editHealthMonitor(id, spec) {
      horizon.modals.status=1;
       var res=apiService.put('/api/lbaas/healthmonitors/' + id + '/', spec);
        res.success(function(response){
              if(response){
                horizon.modals.status=0;
                horizon.modals.spinner.modal('hide');
    ...
    function createHealthMonitor(spec) {
     var res=apiService.post('/api/lbaas/healthmonitors/', spec);
          res.success(function(response){
            if(response){
              horizon.modals.spinner.modal('hide');
            }
