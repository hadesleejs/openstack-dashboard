...
(function () {
  ...
  function MemberDetailsController($scope, $compile, popoverTemplates, patterns, gettext) {
    ...
    function allocateMember(member) {
      var index = $scope.model.members.indexOf(member);
      $scope.model.members.splice(index, 1);
      var newMember = angular.extend(angular.copy(member), { id: member.id});
      $scope.model.spec.members.push(newMember);
    }

    function deallocateMember(member) {
      var availables=$scope.model.availables;
      if(parseInt(member.id)!=member.id){
        for (var x in availables){
          for (var y in availables[x].addresses){
                if((member.id==availables[x].id)||(member.address==availables[x].addresses[y].ip)){
                    $scope.model.members.push(availables[x]);
                    break;
                }
          }
        }
