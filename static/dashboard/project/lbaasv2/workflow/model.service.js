...
(function () {
  ...
  function workflowModel(
    ...
    function initMemberAddresses() {
      ...
      /*可用云主机json数据格式化--add by yangsong*/
      angular.forEach(model.availables, function initAddresses(member) {
        var memberPorts = ports.filter(function filterPortByDevice(port) {
          return port.device_id === member.id;
        });
        member.addresses = [];
    ...
    function setMembersSpec(membersList) {
      ...
      angular.forEach(membersList, function addMember(member) {
        ...
        for (var i=0;i<model.members.length;i++){
          /*获取addresses属性中所有的键值*/
          var keys=[];
          var obj=model.members[i].addresses;
          for (k in obj){
             if (obj.hasOwnProperty(k)){
                  keys.push(k);
             }
          }
          /*获取addresses属性中所有的键值--end*/
            for (var j=0;j<keys.length;j++){
              if(typeof(model.members[i].addresses[keys[j]])!='undefined'){
                  for (var k=0;k<model.members[i].addresses[keys[j]].length;k++){
                      if(model.members[i].addresses[keys[j]][k].addr==member.address){
                          member.addresses=model.members[i].addresses
                          var index = model.members.indexOf(model.members[i]);
                          indexs.push(index);
                      }
                  }
              }
      ...
      model.availables=$.extend(true, [], model.members);
      indexs=indexs.sort();
      indexs.reverse();
      for(var i in indexs){
          model.members.splice(indexs[i], 1);
      }
